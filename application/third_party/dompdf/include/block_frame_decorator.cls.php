...
class Block_Frame_Decorator extends Frame_Decorator {
  ...
  function add_frame_to_line(Frame $frame) {
    ...
    */
    ...
    // var_dump(floatval($w));
    // exit;
    if ( floatval($line["left"]) + floatval($line["w"]) + floatval($line["right"]) + floatval($w) > $this->get_containing_block("w"))
      $this->add_line();
  ...
  function increase_line_width($w) {
    $this->_lines[ $this->_cl ]["w"] += floatval($w);
  }
...
//       return;
    ...
    $y = floatval($this->_lines[$this->_cl]["y"]) + floatval($this->_lines[$this->_cl]["h"]);
